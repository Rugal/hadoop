...
public abstract class OptimizedDataJoinReducerBase extends OptimizedJobBase
{
            ...
            throws IOException
    {
        ...
        List<IntermediateData> smaller = new ArrayList<IntermediateData>();
        this.numOfValues = 0;
        while (values.hasNext())
        {
            ...
            // break;
            // }
            IntermediateData cloned = ((IntermediateData) value).clone(job);
     ...
     * @throws IOException
     */
    private void joinAndCollect(CompositeKey key, List<IntermediateData> smaller,
            IntermediateData value, OutputCollector output, Reporter reporter) throws IOException
    {
        if (smaller.size() < 1)
        {
            IntermediateData combined = combine(key.getKey(), null, value);
            collect(key, combined, output, reporter);
        }
        else
        {
            for (IntermediateData small : smaller)
            {
                IntermediateData combined = combine(key.getKey(), small, value);
                collect(key, combined, output, reporter);
            }
        }
    }
    ...
    private void collect(CompositeKey key, IntermediateData combined, OutputCollector output,
            Reporter reporter) throws IOException
    {
        ...
        if (combined != null)
        {
            ...
            output.collect(null, combined.getData());
     ...
     */
    protected abstract IntermediateData combine(String key, IntermediateData smallValue,
